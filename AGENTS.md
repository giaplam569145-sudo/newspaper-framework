...
- **Test suite**: pytest. Run with `python -m pytest tests/ -v`.
...
src/newspaper/          # Core package
  __init__.py           # Public exports (Newspaper, Article, all generators, exceptions)
  ...
  content/
    ...
    quiz.py             # QuizSystem
    sudoku.py           # SudokuGenerator (saves solution for interactive HTML)
    crossword.py        # CrosswordGenerator (lazy import of pycrossword)
  export/
    base.py             # Exporter ABC
    html_exporter.py    # HtmlExporter (Jinja2 with autoescape)
    json_exporter.py    # JsonExporter
    templates/base.html # Jinja2 template with interactive JS (quiz/sudoku/crossword)
src/api/
...
- `SudokuGenerator` — 9x9 grids. Difficulty: `"easy"` (30 blanks), `"medium"` (40), `"hard"` (50). Saves `solution` (full grid) for interactive validation. `generate()` returns copy via `[row[:] for row in self.grid]`.
- `CrosswordGenerator` — lazy imports `pycrossword`. `generate()` returns `Crossword` dataclass. Raises `NewspaperFrameworkError` if pycrossword not installed.

## Interactive HTML Features

The HTML template (`base.html`) includes inline JavaScript for:
- **Quiz**: "Antworten pruefen" button evaluates radio buttons, highlights correct/incorrect
- **Sudoku**: "Sudoku pruefen" button validates inputs against `data-solution`, restricts input to 1-9
- **Crossword**: "Kreuzwortraetsel pruefen" button validates letter inputs against `data-answer`

## REST API Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/newspaper` | POST | Create newspaper (body: `{"title": "..."}`) |
| `/api/newspaper/<id>/article` | POST | Add article (body: `title`, `content`, optional `force`, `author`, `category`, `priority`) |
| `/api/newspaper/<id>/quiz` | POST | Add quiz (body: `title`, `questions[]`) |
| `/api/newspaper/<id>/sudoku` | POST | Add sudoku (body: `difficulty`) |
| `/api/newspaper/<id>/crossword` | POST | Add crossword (body: `words[]`, `clues{}`) |
| `/api/newspaper/<id>/export` | POST | Export (body: `format` = "html" or "json") |

## Error Classes

- `NewspaperFrameworkError(Exception)` — hard validation errors (empty title, short content, invalid quiz, missing pycrossword).
...
- `pycrossword-generator` is imported lazily in `crossword.py` — the module loads without it.
